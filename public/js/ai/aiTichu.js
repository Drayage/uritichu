...
function handStrength14(hand) {
  ...
  const counts = {};
  for (const c of hand) if (!c.isSpecial) counts[c.numericValue] = (counts[c.numericValue] || 0) + 1;
  for (const v of Object.values(counts)) { if (v === 4) score += 4; else if (v === 3) score += 1; }
  return score;
}

function shouldCallGrandTichu(hand8) { return handStrength8(hand8) >= 10; }
function shouldCallTichu(hand14) { return handStrength14(hand14) >= 14; }
